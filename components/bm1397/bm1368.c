...
void BM1368_send_hash_frequency(float target_freq)
{
    ...
    _send_BM1368((TYPE_CMD | GROUP_ALL | CMD_WRITE), freqbuf, 6, true);
...
static void do_frequency_ramp_up()
{
    ...
    //       bm1368 doesn't get going until after this sequence
...
static void _send_init(uint64_t frequency)
{
    ...
    //      _send_BM1368((TYPE_CMD | GROUP_ALL | CMD_WRITE), init,  6, true);
    ...
    //      _send_BM1368((TYPE_CMD | GROUP_ALL | CMD_WRITE), init2,  6, true);

    //      unsigned char init3[6] =  { 0x00, 0x18, 0xFF, 0x0F, 0xC1, 0x00 };
    //       _send_BM1368((TYPE_CMD | GROUP_ALL | CMD_WRITE), init3,  6, true);
    ...
    //      _send_BM1368((TYPE_CMD | GROUP_ALL | CMD_WRITE), init6,  6, true);

    //      unsigned char init7[6] =  { 0x00, 0x3C, 0x80, 0x00, 0x80, 0x20};
    //       _send_BM1368((TYPE_CMD | GROUP_ALL | CMD_WRITE), init7,  6, true);
    ...
    unsigned char init135[11] = {0x55, 0xAA, 0x51, 0x09, 0x00, 0x3C, 0x80, 0x00, 0x8B, 0x00, 0x12};
    _send_simple(init135, 11);

    unsigned char init136[11] = {0x55, 0xAA, 0x51, 0x09, 0x00, 0x3C, 0x80, 0x00, 0x80, 0x18, 0x1F};
    ...
    unsigned char init171[11] = {0x55, 0xAA, 0x41, 0x09, 0x00, 0x2C, 0x00, 0x7A, 0x00, 0x03, 0x19};
    _send_simple(init171, 11);

  // This is UART so you want to set this right, 
    ...
    unsigned char init176[11] = {0x55, 0xAA, 0x41, 0x09, 0x00, 0x3C, 0x80, 0x00, 0x8B, 0x00, 0x1A};
    _send_simple(init176, 11);
    unsigned char init177[11] = {0x55, 0xAA, 0x41, 0x09, 0x00, 0x3C, 0x80, 0x00, 0x80, 0x18, 0x17};
    ...
    unsigned char init179[11] = {0x55, 0xAA, 0x41, 0x09, 0x00, 0x0C, 0x80, 0x00, 0x00, 0x00, 0x1A};
    _send_simple(init179, 11);
    ...
    unsigned char init794[11] = {0x55, 0xAA, 0x51, 0x09, 0x00, 0x10, 0x00, 0x00, 0x15, 0xA4, 0x0A};
